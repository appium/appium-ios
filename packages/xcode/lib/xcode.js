...
async function getConnectedDevices () {
  ...
  let [stdout] = await exec('/usr/sbin/system_profiler -xml SPUSBDataType', {maxBuffer: 524288, timeout: XCODE_SELECT_TIMEOUT});
  ...
  while (entriesToSearch.length > 0) {
    ...
    } else if (currentEntry._name && currentEntry._name.substring(0, 4) === "iPad" ||
      currentEntry._name && currentEntry._name.substring(0, 6) === "iPhone") {
      let deviceInfo = {
        name: currentEntry._name,
        udid: currentEntry.serial_num,
        product_id: currentEntry.product_id,
        device_version: currentEntry.bcd_device
      };
